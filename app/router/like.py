...
@router.delete('/post/unlike/{pid}')
def unlikePost(pid: int, db: Session = Depends(get_db), user: oauth2.get_current_user = Depends()):
    user_id  = user.dict().get('user_id')
    get_user = db.query(models.userCred).filter(models.userCred.email==user_id).first()
    if get_user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid Credentials")
        ...
        if get_post is None:
            ...
            elif check_for_like is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid Operation")

            db.delete(check_for_like)
            db.commit()
            raise HTTPException(status_code=status.HTTP_202_ACCEPTED, detail="Post is unliked")
